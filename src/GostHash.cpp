...
void addMod512(const uint8_t* a, const uint8_t* b, uint8_t* c) {
	for (int t = 0, i = 63; i >= 0; --i) {
		t = a[i] + b[i] + (t >> 8);
		c[i] = t & 0xFF;
	}
}
...
void addXor512(const void* a, const void* b, void* c) {
	const int64_t* A = static_cast<const int64_t*>(a), *B = static_cast<const int64_t*>(b);
	uint64_t* C = static_cast<uint64_t*>(c);
...
void keySchedule(uint8_t* K, const int& i) {
	addXor512(K, cTable[i], K);
...
void g_N(const uint8_t* N, uint8_t* h, const uint8_t* m) {
	uint8_t t[64], K[64];
...
void hash_X(uint8_t* IV, const uint8_t* message, const uint64_t& length, uint8_t* out) {
	uint8_t v512[64] = {
...
void gosthash::hash256(const uint8_t* message, const uint64_t& length, uint8_t* out) {
